...
ne_solar, ge_solar = [], []
df_ne = pd.DataFrame(index=range(2011, 2023), columns=country_solar)
df_ge = pd.DataFrame(index=range(2011, 2023), columns=country_solar)
...
cost_red_mp = (cost_const - cost_price) * capa_global  # 2010-2022, added value
...
x1labels = ["N G\n'"+str(t)[2:4] if t<=2022 else "" for t in x][:12]
x1labels.append("'30E")
...
width=0.3
base = np.array([0] * 12, dtype='float')
ge_other, ne_other = 0, 0
for i in range(len(country_solar)):
    if country_solar[i] == 'Others':
        ne_other += ne_solar[i]
    plt.bar(x[:12]-width/2, ne_solar[i], bottom=base, width=width, color=cmap[country_solar[i]], alpha=0.8, edgecolor='k')
    base += ne_solar[i]
base = np.array([0] * 12, dtype='float')
for i in range(len(country_solar)):
    if country_solar[i] == 'Others':
        ge_other += ge_solar[i]
    plt.bar(x[:12] + width / 2, ge_solar[i], bottom=base, width=width, color=cmap[country_solar[i]], label=country_solar[i], alpha=0.8, edgecolor='k')
    base += ge_solar[i]
plt.xticks(x2[:-1], x1labels[:-1])
plt.ylim(0, 800000)
plt.yticks(np.linspace(0, 800000, 5), ['0', '2', '4', '6', '8'])
plt.ylabel('Annual cost savings ($ 100 billion)')
plt.xlabel('Year')
plt.text(0, 840000, 'Solar PV')
plt.text(-1.2, 840000, 'a.', weight='bold', fontsize=15)
...
width = 0.2
lns1 = ax1.bar(x[:12], y[1:], width, alpha=0.6, label="Material price (MP)", color='grey', edgecolor='k')
lns2 = plt.bar(x[:12] + width, saving_solar['NEing'], width, alpha=0.6, label="Savings in developing countries by NE", color='skyblue', edgecolor='k', hatch='////')
lns3 = plt.bar(x[:12] + 2*width, saving_solar['GEing'], width, alpha=0.6, label="Savings in developing countries by GE", color='green', edgecolor='k', hatch='////')
lns4 = plt.bar(x[:12] + width, saving_solar['NEed'], width, alpha=0.6, label="Savings in developed countries by NE", color='skyblue', edgecolor='k', hatch='\\\\\\\\', bottom=saving_solar['NEing'])
lns5 = plt.bar(x[:12] + 2*width, saving_solar['GEed'], width, alpha=0.6, label="Savings in developed countries by GE", color='green', edgecolor='k', hatch='\\\\\\\\', bottom=saving_solar['GEing'])
plt.bar(x[-1], cost_red_mp[-1] / capa_global[-1] * cap_global_cum[-1] * 2 + cost_red_mp[-1], 
width, alpha=0.6, color='grey', edgecolor='k', linestyle='dashed')
plt.bar(x[-1] + width, cost_red_ne[-1] / capa_global[-1] * cap_global_cum[-1] * 2 + cost_red_ne[-1],
width, alpha=0.6, color='skyblue', edgecolor='k', linestyle='dashed')
NE += cost_red_ne[-1] / capa_global[-1] * cap_global_cum[-1] * 2 + cost_red_ne[-1]
GE += cost_red_ge[-1] / capa_global[-1] * cap_global_cum[-1] * 2 + cost_red_ge[-1]
plt.bar(x[-1] + width*2, cost_red_ge[-1] / capa_global[-1] * cap_global_cum[-1] * 2 + cost_red_ge[-1],
...
ax2 = ax1.twinx()
x3 = x2+width/2
lns6, = ax2.plot(x3[:-1]+width/2, cap_global_cum[1:13], color='k', marker='.', label='Cumulative installed capacity')
ax2.plot(x3[-2:]+width/2, [cap_global_cum[-1], cap_global_cum[-1]*3], color='k', marker='.', linestyle=':')
...
ne_wind, ge_wind = [], []
df_ne = pd.DataFrame(index=range(2011, 2023), columns=country_wind)
df_ge = pd.DataFrame(index=range(2011, 2023), columns=country_wind)
for c in country_wind:
    ...
    CostSum_self += cost_self_c * capa_c
    ne_wind.append(((np.array([cost_sim_c[0]]*13) - cost_self_c) * capa_c)[1:])
    ge_wind.append(((cost_self_c - cost_sim_c) * capa_c)[1:])
    df_ne.loc[:, c] = (np.array([cost_sim_c[0]]*13) - cost_self_c)[1:]
    df_ge.loc[:, c] = (cost_self_c - cost_sim_c)[1:]
cost_sim = CostSum_sim / capa_global
cost_self = CostSum_self / capa_global
cost_const = np.array([cost_sim[0]]*13)
...
for i in range(len(country_wind)):
    if country_wind[i] == 'Others':
        ne_other += ne_wind[i]
    plt.bar(x[:12]-width/2, ne_wind[i], bottom=base, width=width, color=cmap[country_wind[i]], alpha=0.8, edgecolor='k')
    base += ne_wind[i]
base = np.array([0] * 12, dtype='float')
for i in range(len(country_wind)):
    if country_wind[i] == 'Others':
        ge_other += ge_wind[i]
    plt.bar(x[:12] + width / 2, ge_wind[i], bottom=base, width=width, color=cmap[country_wind[i]], label=country_wind[i], alpha=0.8, edgecolor='k')
    base += ge_wind[i]
plt.xticks(x2[:-1], x1labels[:-1])
plt.ylim(0, 60000)
plt.yticks(np.linspace(0, 60000, 7), ['0', '1', '2', '3', '4', '5', '6'])
plt.ylabel('Annual cost savings ($ 10 billion)')
plt.xlabel('Year')
plt.text(0, 60000*1.05, 'Wind power')
plt.text(-1.2, 60000*1.05, 'b.', weight='bold', fontsize=15)
...
x = np.arange(len(x)) + 1
width = 0.3
lns1 = plt.bar(x[:12], saving_wind['NEing'], width, alpha=0.6, label="Savings in developing countries by NE", color='skyblue', edgecolor='k', hatch='////')
lns2 = plt.bar(x[:12] + width, saving_wind['GEing'], width, alpha=0.6, label="Savings in developing countries by GE", color='green', edgecolor='k', hatch='////')
lns3 = plt.bar(x[:12], saving_wind['NEed'], width, alpha=0.6, label="Savings in developed countries by NE", color='skyblue', edgecolor='k', hatch='\\\\\\\\', bottom=saving_wind['NEing'])
lns4 = plt.bar(x[:12] + width, saving_wind['GEed'], width, alpha=0.6, label="Savings in developed countries by GE", color='green', edgecolor='k', hatch='\\\\\\\\', bottom=saving_wind['GEing'])
plt.bar(x[-1], cost_red_ne[-1] / capa_global[-1] * cap_global_cum[-1] * 2 + cost_red_ne[-1], width,
alpha=0.6, color='skyblue',edgecolor='k', linestyle='dashed')
plt.bar(x[-1] + width, cost_red_ge[-1] / capa_global[-1] * cap_global_cum[-1] * 2 + cost_red_ge[-1], width,
...
ax2 = ax1.twinx()
x3 = x2+width/2
lns5, = ax2.plot(x3[:-1], cap_global_cum[1:13], color='k', marker='.', label='Cumulative installed capacity')
ax2.plot(x3[-2:], [cap_global_cum[-1], cap_global_cum[-1]*3], color='k', marker='.', linestyle=':')
...
plt.bar(start+width, df['scn2'].values[0], width, alpha=0.6, label="Without GE scenario", color='green',
        bottom=df['scn1'].values[1], edgecolor='k', linestyle='dashed')
